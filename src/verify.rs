...
fn hex_to_byte(hex: &str) -> Result<Vec<u8>, String> {
    ...
    (0..hex.len())
        ...
        .map_err(|_| "Invalid hex string encountered during decoding".to_string())
}
...
pub fn priv_to_pub(priv_key: &str) -> Result<VerifyingKey, String> {
    ...
    let priv_key = SigningKey::from_bytes(&priv_key_bytes)
        .map_err(|_| "Key exchange: Invalid private key".to_string())?;
    let pub_key = VerifyingKey::from(&priv_key);
...
pub fn sign(priv_key: &str, hash: &str) -> Result<Signature, String> {
    ...
    let priv_key_bytes = hex_to_byte(priv_key)?;
    let priv_key = SigningKey::from_bytes(&priv_key_bytes)
        .map_err(|_| "Signing: Invalid private key".to_string())?;
...
pub fn verify(pub_key: &str, hash: &str, signature: &str) -> Result<bool, String> {
    ...
    let pub_key = VerifyingKey::from_sec1_bytes(&pub_key_bytes)
        .map_err(|_| "Verifying: Invalid public key".to_string())?;

    let signature_bytes = hex_to_byte(signature)?;
    let signature = Signature::try_from(signature_bytes.as_slice()).map_err(|_| "Verifying: Invalid signature".to_string())?;
